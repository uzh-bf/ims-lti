...
exports.shouldBehaveLikeNonce = newStore => {
  ...
  describe('.isNew', () => {
    ...
    it('should return true for time-relivant nonces', done => {
      ...
      const first_test = () => store.isNew('tr-00', now, function(err, valid) {
        should.not.exist(err);
        valid.should.equal(true);
        return second_test();
      });
      var second_test = () => store.isNew('tr-11', future, function(err, valid) {
        should.not.exist(err);
        valid.should.equal(true);
        return third_test();
      });
      var third_test = () => store.isNew('tr-01', past_minute, function(err, valid) {
        should.not.exist(err);
        valid.should.equal(true);
        return fourth_test();
      });
      var fourth_test = () => store.isNew('tr-02', past_two_minutes, function(err, valid) {
        should.not.exist(err);
        valid.should.equal(true);
        return done();
      });
    ...
    return it('should return false for expired nonces', done => {
      ...
      const first_test = () => store.isNew('00', five_and_one_sec_old, function(err, valid) {
        should.exist(err);
        valid.should.equal(false);
        return second_test();
      });
      var second_test = () => store.isNew('11', hour_old, function(err, valid) {
        should.exist(err);
        valid.should.equal(false);
        return done();
      });
  ...
  return describe('.setUsed', () => {
    ...
    return it('should set nonces to used', done => {
      const store = newStore();
      const now = Math.round(Date.now()/1000);
      return store.setUsed('11', now, () => store.isNew('11', now+1, function(err, valid) {
        should.exist(err);
        valid.should.equal(false);
        return done();
      }));
